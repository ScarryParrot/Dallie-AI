...
const Card = ({_id,photo,prompt,name}) => {
  ...
  return (
    <div className="mt-10 rounded-xl group relative shadow-card hover:shadow-cardhover card">
      <img
      ...
      <div className="group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 right-0 bg-[#10131f] m-2 p-4 rounded-md">
        ...
        <div className="mt-5 flex justify-between items-center gap-2">
          ...
          <button type="button" onClick={() => downloadImage(_id, photo)} className="outline-none bg-transparent border-none">
            <img src={download} alt="download" onClick={dwldClick} className="w-6 h-6 object-contain invert" />
          </button>
        </div>
      </div>
    </div>
